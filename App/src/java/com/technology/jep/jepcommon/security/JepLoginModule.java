...
public class JepLoginModule implements LoginModule {
  ...
  public boolean login() throws LoginException {
    ...
    icb = new IdentityCallback();
    //Определим массив параметров для обработки информации от пользователя.
    ...
    if (!icb.isIdentityAsserted()) {  // Обработка вызова из Java SSO.
      //Получим логин и пароль предоставленные пользователем.
    ...
    } else {  // Обработка после javaSSO: получение username и уже по username всего остального
      //Получим логин пользователя.
      this.username = icb.getIdentity();
      try {
...
//      System.out.println(this.getClass() + ".fillPrincipals(" + operatorId + "): jepPrincipal = " + jepPrincipal);
  ...
  private String getPassword(Callback[] callbacks) {
    String password = null;
    char[] charPassword = ((PasswordCallback) callbacks[2]).getPassword();
