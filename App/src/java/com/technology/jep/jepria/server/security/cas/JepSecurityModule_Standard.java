...
/**
 * Стандартный модуль поддержки безопасности (Tomcat, WebLogic, ...)
 */
public class JepSecurityModule_Standard extends JepAbstractSecurityModule {

  static {
    logger = Logger.getLogger(JepSecurityModule_Standard.class.getName());
  }
  ...
  public static synchronized JepSecurityModule getInstance(HttpServletRequest request) {
    ...
    JepSecurityModule_Standard securityModule;
    if(principal == null) { // Работает гость ?
      securityModule = (JepSecurityModule_Standard) session.getAttribute(JEP_SECURITY_MODULE_ATTRIBUTE_NAME);
      if(securityModule == null) { // Первый вход ?
        securityModule = new JepSecurityModule_Standard();
